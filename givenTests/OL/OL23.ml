...
let print_expr = function (exp : expression) ->
  ...
  let close_paren prec op_prec =
    (if prec > op_prec then print_string ")") in
  let rec (print_ : int -> expression -> unit) =
   fun prec (exp : expression) -> ( (* prec is the current precedence *)
    ...
    | Sum(f, g) ->
        ((open_paren prec 0);
        (print_ 0 f); (print_string " + "); (print_ 0 g);
        (close_paren prec 0))
    | Diff(f, g) ->
        ((open_paren prec 0);
        (print_ 0 f); (print_string " - "); (print_ 1 g);
        (close_paren prec 0))
    | Prod(f, g) ->
        ((open_paren prec 2);
        (print_ 2 f); (print_string " * "); (print_ 2 g);
        (close_paren prec 2))
    | Quot(f, g) ->
        ((open_paren prec 2);
        (print_ 2 f); (print_string " / "); (print_ 3 g);
        (close_paren prec 2)))
  in (print_ 0 exp);;
