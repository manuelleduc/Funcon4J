...
public class CamlLightTruffle {
    ...
    public static IValue eval(String src) throws FunconException {
        final funcons.algebras.AllAlg<CLStatementNode> evalAlg = new TruffleAllFactory() {
        };
//        return eval(src, () -> evalAlg);
        return null;
    }

    public static CLStatementNode eval(String src, camllight.algebras.AllAlg alg) throws FunconException {
        Recorder builder = parse(src, Recorder.create(camllight.algebras.AllAlg.class));
        CLStatementNode eval = builder.build(alg);
//        CLStatementNode env = importStandardLibrary(ValueFactory.getInstance().mapWriter().done());
    ...
    public static IValue importStandardLibrary(IValue env, Set<String> excludes) throws FunconException {
        ...
        for (Method m : lib.getClass().getMethods()) {
            ...
            try {
                ...
                env = alg.mapUnion(
                        (e, g) -> env2,
                        alg.bindValue(alg.id(methodName), (IEval) m.invoke(lib))
